...
import { ExternalLink, TrendingDown, ArrowUpRight, ShieldCheck, Tag } from 'lucide-react';
  ...
  return (
    <div className="group bg-zinc-900/60 border border-zinc-800/80 rounded-2xl overflow-hidden hover:border-zinc-700/80 hover:shadow-xl transition-all duration-300 hover:scale-[1.01] flex flex-col h-full">
      ...
      <div className="relative aspect-[4/3] bg-zinc-950 overflow-hidden w-full shrink-0">
        ...
        {score !== null && (
          <span
            className={`absolute top-2.5 right-2.5 flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-lg border backdrop-blur-md ${getScoreStyle(
              score
            )}`}
          >
      ...
      <div className="p-4 flex-1 flex flex-col justify-between gap-3">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {listing.condition_label && (
              <span className="bg-zinc-800/40 text-zinc-400 px-2 py-0.5 rounded text-[10px] font-medium border border-zinc-800/50">
                {listing.condition_label}
              </span>
            )}
          </div>
          ...
          <Link
            ...
            className="block text-sm font-semibold text-zinc-100 hover:text-indigo-400 transition-colors line-clamp-1 mt-1"
          >
        ...
        <div className="space-y-3 pt-2 border-t border-zinc-800/50">
          ...
          <div className="flex gap-2 pt-1">
            <Link
              href={`/listings/${listing.id}`}
              className="flex-1 text-center bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs py-2 rounded-xl transition-colors font-medium border border-zinc-700/30 flex items-center justify-center gap-1 group-hover:border-zinc-600"
            >
              Détails
            ...
            {listing.url && (
              <a
                ...
                className="bg-indigo-600 hover:bg-indigo-500 text-white p-2 rounded-xl transition-all duration-200 flex items-center justify-center glow-indigo"
                title="Ouvrir sur Vinted"
              >
